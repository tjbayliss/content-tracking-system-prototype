...
*/
	...
	function wrap(text, content_width, marginLeft) {
		  text.each(function() {
		    var text = d3.select(this),
		        ...
		        tspan = text.text(null).append("tspan").attr("x", marginLeft).attr("y", y).attr("dy", dy + "em")
		    while (word = words.pop()) {
		      ...
		      if (tspan.node().getComputedTextLength() > content_width) {
		        ...
		        tspan.text(line.join(" "))
		        line = [word]
		        tspan = text.append("tspan").attr("x", marginLeft).attr("y", y).attr("dy", `${++lineNumber * lineHeight + dy}em`).text(word)
		      }
		    }
		  })
		}
...
// https://gist.githubusercontent.com/abenrob/787723ca91772591b47e/raw/8a7f176072d508218e120773943b595c998991be/world-50m.json
	 ...
	 function alertSize(){
		...
		if( tjb.diagnostics ){ console.log( 'Width = ' + myWidth + " : " + 'Height = ' + myHeight ); }
	...
	function BrowserDetection(){
		...
		if (navigator.userAgent.search("MSIE") >= 0) {  tjb.browser = "MSIE"; }
		//Check if browser is Chrome
		else if (navigator.userAgent.search("Chrome") >= 0) {  tjb.browser = "Chrome"; }
		//Check if browser is Firefox 
		else if (navigator.userAgent.search("Firefox") >= 0) {  tjb.browser = "Firefox"; }
		//Check if browser is Safari
		else if (navigator.userAgent.search("Safari") >= 0) {  tjb.browser = "Safari"; }
		//Check if browser is Opera
		else if (navigator.userAgent.search("Opera") >= 0) {  tjb.browser = "Opera"; }

		if( tjb.diagnostics ){ console.log( 'Browser: ' + tjb.browser ); }
	...
	function mediaType(){
		...
		|| navigator.userAgent.match(/Windows Phone/i)
			...
			tjb.mediaType = "mobile";
		}
		else if (navigator.userAgent.match(/iPad/i) ){
			tjb.mediaType = "tablet";
		}
		else {
			tjb.mediaType = "desktop";
		}

		tjb.mediaType = tjb.mediaType;

		return;

	}// end function mediaType
	...
	function checkOrientation(){
        ...
        if (isAndroid) {
            ...
            if (window.orientation == 0 || window.orientation == 180) { //Landscape Mode
                console.log("Landscape Mode - Android")
            }
            else if (window.orientation == 90 || window.orientation == -90) { //Portrait Mode
                console.log("Portrait Mode - Android")
            }
            else{
           		console.log("Catch All - Android")
            }
        }
        else {
            if (window.orientation == 90 || window.orientation == -90) { //Landscape Mode
                console.log("Landscape Mode - iOS")
            }
            else if (window.orientation == 0 || window.orientation == 180) { //Portrait Mode
                console.log("Portrait Mode - iOS")
            }
            else{
           		console.log("Catch All - iOS")
            }
        }
